...
function getESBuildConfig(param, platform, aliases) {
  var { entryPoints, resolvePlatformSpecificEntries, ...options } = param, _process_env_DEBUG;
  ...
  var resolvedEntryPoints = resolvePlatformSpecificEntries ? entryPoints.map(import_loadTamagui.resolveWebOrNativeSpecificEntry) : entryPoints, res = {
    bundle: !0,
...
async function esbundleTamaguiConfig(props, platform, aliases) {
  ...
  var config = getESBuildConfig(props, platform, aliases);
  return await import_esbuild.default.build(config);
}
async function asyncLock(props) {
  var lockFile = (0, import_node_path.join)((0, import_node_path.dirname)(props.outfile), (0, import_node_path.basename)(props.outfile, ".lock")), lockStat = await FS.stat(lockFile).catch(function() {
  }), lockedMsAgo = lockStat ? (/* @__PURE__ */ new Date()).getTime() - new Date(lockStat.mtime).getTime() : Number.POSITIVE_INFINITY;
  if (lockedMsAgo < 500) {
    var _process_env_DEBUG;
    !((_process_env_DEBUG = process.env.DEBUG) === null || _process_env_DEBUG === void 0) && _process_env_DEBUG.startsWith("tamagui") && console.info("Waiting for existing build", props.entryPoints);
    for (var tries = 5; tries--; ) {
