...
var import_colors = require("@tamagui/colors"), import_theme_builder = require("@tamagui/theme-builder"), import_web = require("@tamagui/web"), import_utils = require("./utils");
const colorTokens = {
  light: {
  ...
  const transparent = (hsl, opacity = 0) => hsl.replace("%)", `%, ${opacity})`).replace("hsl(", "hsla("), getColorPalette = (colors, accentColors) => {
    ...
    color.white0,
    brandColor.light
  ], lightColorNames = (0, import_utils.objectKeys)(colorTokens.light), lightPalettes = (0, import_theme_builder.objectFromEntries)(
    lightColorNames.map(
      (key, index) => [
        `light_${key}`,
        getColorPalette(
  ...
  ), darkColorNames = (0, import_utils.objectKeys)(colorTokens.dark), darkPalettes = (0, import_theme_builder.objectFromEntries)(
    darkColorNames.map(
      (key, index) => [
        `dark_${key}`,
        getColorPalette(
  ...
  return {
    ...(0, import_theme_builder.objectFromEntries)(
      (0, import_utils.objectKeys)(lightTemplates).map(
        (name) => [`light_${name}`, lightTemplates[name]]
      )
    ),
    ...(0, import_theme_builder.objectFromEntries)(
      (0, import_utils.objectKeys)(darkTemplates).map(
