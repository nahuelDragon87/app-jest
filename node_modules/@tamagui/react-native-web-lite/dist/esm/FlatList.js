...
class FlatList extends React.PureComponent {
  ...
  _checkProps(props) {
    ...
    invariant(!getItem && !getItemCount, "FlatList does not support custom data formats."), numColumns > 1 ? invariant(!horizontal, "numColumns does not support horizontal.") : invariant(
      !columnWrapperStyle,
  ...
  _keyExtractor = (items, index) => {
    const numColumns = numColumnsOrDefault(this.props.numColumns), keyExtractor = this.props.keyExtractor ?? defaultKeyExtractor;
    return numColumns > 1 ? (invariant(
      Array.isArray(items),
  ...
  _renderer = (ListItemComponent, renderItem, columnWrapperStyle, numColumns, extraData) => {
    const cols = numColumnsOrDefault(numColumns), render = (props) => ListItemComponent ? /* @__PURE__ */ jsx(ListItemComponent, { ...props }) : renderItem ? renderItem(props) : null, renderProp = (info) => {
      if (cols > 1) {
        const { item, index } = info;
        return invariant(Array.isArray(item), "Expected array of items with numColumns > 1"), /* @__PURE__ */ jsx(View, { style: [styles.row, columnWrapperStyle], children: item.map((it, kk) => {
